...
This keeps a host that reconnects after a dropped link from being locked out by its own dead
connection, which the device's network stack may take minutes to notice. It also means two
hosts that both reconnect on their own take the device from each other in turn. A host should
